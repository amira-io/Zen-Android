package io.thera.zen.layout.slider;
...
public class ZenViewBehind extends ViewGroup {
    ...
    private static final String TAG = "ZenViewBehind";

    private static final int MARGIN_THRESHOLD = 48; // dips
    private int mTouchMode = ZenSlidingMenu.TOUCHMODE_MARGIN;

    private ZenViewAbove mViewAbove;
    ...
    public ZenViewBehind(Context context) {
        this(context, null);
    }

    public ZenViewBehind(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setZenViewAbove(ZenViewAbove zenViewAbove) {
        mViewAbove = zenViewAbove;
    }
    ...
    public void setMode(int mode) {
        if (mode == ZenSlidingMenu.LEFT || mode == ZenSlidingMenu.RIGHT) {
            if (mContent != null)
    ...
    public int getMenuPage(int page) {
        ...
        if (mMode == ZenSlidingMenu.LEFT && page > 1) {
            return 0;
        } else if (mMode == ZenSlidingMenu.RIGHT && page < 1) {
            return 2;
        } else {
            return page;
        }
    ...
    public int getAbsLeftBound(View content) {
        if (mMode == ZenSlidingMenu.LEFT || mMode == ZenSlidingMenu.LEFT_RIGHT) {
            return content.getLeft() - getBehindWidth();
        } else if (mMode == ZenSlidingMenu.RIGHT) {
            return content.getLeft();
        }
    ...
    public int getAbsRightBound(View content) {
        if (mMode == ZenSlidingMenu.LEFT) {
            return content.getLeft();
        } else if (mMode == ZenSlidingMenu.RIGHT || mMode == ZenSlidingMenu.LEFT_RIGHT) {
            return content.getLeft() + getBehindWidth();
        }
        return 0;
    }
    ...
    public boolean menuOpenTouchAllowed(View content, int currPage, float x) {
        switch (mTouchMode) {
            case ZenSlidingMenu.TOUCHMODE_FULLSCREEN:
                return true;
            case ZenSlidingMenu.TOUCHMODE_MARGIN:
    ...
    public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
        if (mMode == ZenSlidingMenu.LEFT || (mMode == ZenSlidingMenu.LEFT_RIGHT && currPage == 0)) {
            return x >= content.getLeft();
        } else if (mMode == ZenSlidingMenu.RIGHT || (mMode == ZenSlidingMenu.LEFT_RIGHT && currPage == 2)) {
